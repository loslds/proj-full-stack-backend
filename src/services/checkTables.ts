...
export async function checkTables(): Promise<{
  ...
  try {
    ...
    for (const table of requiredTables) {
      try {
        let count = 0;
        const tableInfo = tableCreationMap[table];

        if (!existingTables.includes(table)) {
          if (tableInfo) {
            const repoInstance = new tableInfo.repoClass(dbSource);
            await repoInstance[tableInfo.createMethod]();
            steps.push(`✅ ${table} criada com sucesso.`);
            existingTables.push(table);

            if (tableInfo.insertDefaults) {
              await tableInfo.insertDefaults();
            }
          } else {
    ...
    // Atualiza tabela de controle 'systables'
...
//           if (table === 'pessoas') {
//             await repoInstance.insertDefaultPessoas();
