...
export async function checkTables(): Promise<{
  ...
  if (!AppDataSource.isInitialized) {
    ...
    steps.push('✅ DataSource inicializado');
  }
  ...
  try {
    ...
    steps.push('⏳ Verificando tabelas obrigatórias...');
    await delay(300);
    ...
    for (const table of requiredTables) {
      ...
      if (!config) {
        steps.push(`❌ Configuração inexistente para ${table}`);
        return { success: false, message: 'Erro de configuração', steps };
      }
      ...
      if (!existingTables.includes(table)) {
        ...
        steps.push(`✅ Tabela ${table} criada`);
      } else {
        steps.push(`✅ Tabela ${table} já existe`);
      }

      const count = (await repo[config.findAll]({})).length;
      steps.push(`ℹ️ ${table} possui ${count} registros`);
