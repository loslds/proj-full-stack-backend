...
export async function checkTables(): Promise<{
  ...
  try {
      ...
      try {
        ...
        if (existingTables.includes(table)) {
          if (table === 'systables') {
            ...
            count = (await repo.findSystableAll({})).length;
    ...
    try {
      ...
      steps.push(`❌ Erro geral no sincronismo systables: ${error.message || error}`);
    }
    ...
    return {
      ...
      checkedTables,
      systablesRecords: await new SystablesRepository(dbSource).findSystableAll({}),
