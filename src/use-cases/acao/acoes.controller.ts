...
// C:\repository\proj-full-stack-backend\src\use-cases\acao\acoes.controller.ts
...
export class AcoesController {
    ...
    try {
      ...
      if (!nome || !abrev || !cor || !nivel) {
        throw new HttpException(
          400,
          'Nome, abrev, cor e nivel são obrigatórios'
        );
      }
  ...
  async searchAcoesAll(req: Request, res: Response, next: NextFunction) {
    try {
      const { id, nome, abrev, cor, nivel } = req.query;

      const acoes = await this.acoesRepository.searchAcoes({
        id: id !== undefined ? Number(id) : undefined,
        nome: nome !== undefined ? String(nome) : undefined,
        abrev: abrev !== undefined ? String(abrev) : undefined,
        cor: cor !== undefined ? String(cor) : undefined,
  ...
  /** GET → Busca por cor aproximada */
  async searchAcoesCor(req: Request, res: Response, next: NextFunction) {
    try {
      const text = req.query.text !== undefined ? String(req.query.text) : undefined;

      const acoes = await this.acoesRepository.searchCorAcoes(text);
  ...
  async findOneAcoesAbrev(req: Request, res: Response, next: NextFunction) {
    try {
      const abrev = req.query.abrev !== undefined ? String(req.query.abrev) : undefined;
